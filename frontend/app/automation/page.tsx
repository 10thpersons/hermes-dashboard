...
function StatusBadge({ status }: { status?: string }) {
  ...
  const label = status === "success" ? "success" : status === "failed" || status === "error" ? "failed" : "never";
...
export default function AutomationPage() {
  ...
  const [viewModal, setViewModal] = useState<{ jobId: string; filename?: string } | null>(null);
      ...
      <div className="flex items-center justify-between">
        ...
        {Array.isArray(jobs) && jobs.length > 0 && (
          <span className="text-sm text-[var(--text-secondary)]">
            {jobs.length} jobs · {jobs.filter((j: CronJob) => j.enabled).length} active
          </span>
        )}
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={7} className="px-4 py-8 text-center text-[var(--text-secondary)]">No cron jobs</td></tr>
            ) : (
                  ...
                  <Fragment key={job.id}>
                    ...
                    >
                      <td className="px-4 py-3 font-medium">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-[var(--text-secondary)] font-mono">{job.id}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-[var(--text-secondary)] font-mono text-xs">{scheduleLabel(job)}</td>
                      <td className="px-4 py-3 text-[var(--text-secondary)] text-xs">{job.model || "—"}</td>
                      <td className="px-4 py-3 text-[var(--text-secondary)] text-xs">{fmtRelative(job.last_run_time ?? job.last_run_at)}</td>
                      <td className="px-4 py-3"><StatusBadge status={job.last_run_status} /></td>
                      ...
                      <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                        ...
                        >
                          View Output{job.output_file_count ? ` (${job.output_file_count})` : ""}
                        </button>
                      </td>
                    </tr>
                    {isExpanded && (
                      <tr key={`${job.id}-expanded`} className="bg-[var(--bg-tertiary)]">
...
function ExpandedPanel({ jobId }: { jobId: string }) {
  ...
  if (isLoading) return <div className="text-sm text-[var(--text-secondary)] py-4">Loading latest output...</div>;
  if (error || !output) return <div className="text-sm text-[var(--text-secondary)] py-4">No output available for this job.</div>;

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-[var(--text-secondary)]">
        <span>Latest: <span className="font-mono">{output.filename}</span></span>
        <span>{fmtSize(output.size)} · {fmtRelative(output.mtime_iso)}</span>
      </div>
      <pre className="max-h-80 overflow-auto rounded-lg border border-[var(--border)] bg-[var(--bg-primary)] p-3 text-xs font-mono whitespace-pre-wrap break-words">
        {output.content}
        {output.truncated && <span className="text-[var(--text-secondary)]">\n\n[output truncated]</span>}
      </pre>
    </div>
  );
}
...
function OutputModal({
  ...
  const [selectedFile, setSelectedFile] = useState<string | undefined>(initialFilename);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
          <h2 className="text-sm font-semibold">Cron Output · <span className="font-mono text-[var(--text-secondary)]">{jobId}</span></h2>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="w-56 shrink-0 border-r border-[var(--border)] overflow-y-auto">
            ...
            {histLoading ? (
              <div className="px-3 py-4 text-xs text-[var(--text-secondary)]">Loading...</div>
              ...
              history.files.map((f: HistoryFile) => (
                ...
                >
                  ...
                  <div className="text-[var(--text-secondary)] mt-0.5">{fmtRelative(f.mtime_iso)} · {fmtSize(f.size)}</div>
                </button>
              ))
            ) : (
              <div className="px-3 py-4 text-xs text-[var(--text-secondary)]">No output files</div>
            )}
          </div>
          {/* Content */}
          <div className="flex-1 min-w-0 flex flex-col">
            {outLoading ? (
              <div className="flex-1 flex items-center justify-center text-sm text-[var(--text-secondary)]">Loading...</div>
            ) : output ? (
              <>
              ...
              <div className="flex-1 flex items-center justify-center text-sm text-[var(--text-secondary)]">No output</div>
            )}
          </div>
        </div>
      </div>
