...
export default function KnowledgePage() {
  ...
  const { data: memory } = useQuery({ queryKey: ["memory"], queryFn: fetchers.memory, enabled: tab === "memory" });
  const { data: skills } = useQuery({ queryKey: ["skills"], queryFn: fetchers.skills, enabled: tab === "skills" });
  const { data: souls } = useQuery({ queryKey: ["souls"], queryFn: fetchers.souls, enabled: tab === "souls" });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)] max-h-[70vh] overflow-y-auto">
          {items?.length === 0 ? (
            <div className="p-4 text-sm text-[var(--text-secondary)]">No items found</div>
          ) : (
            items?.map((item: any) => (
              <button
                key={item.name}
              ...
              >
                ...
                {item.relative && (
                  <div className="text-xs text-[var(--text-secondary)] mt-0.5">{item.relative}</div>
                )}
              </button>
            ))
          )}
            ...
            <>
              ...
              {selectedItem.size > 10000 && (
                <div className="mt-4 text-xs text-[var(--warning)] border-t border-[var(--border)] pt-3">
                  ⚠ Content truncated. Full size: {selectedItem.size.toLocaleString()} chars. Showing first 10,000.
                </div>
              )}
            </>
          ) : (
