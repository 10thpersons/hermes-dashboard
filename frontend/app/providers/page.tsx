...
export default function ProvidersPage() {
  const { data: provData, isLoading: provLoading, error: provError } = useQuery<ProvidersResponse>({
    queryKey: ["providers"],
    queryFn: fetchers.providers,
    refetchInterval: 30000,
  });

  const { data: modelData, isLoading: modelsLoading } = useQuery<ModelsResponse>({
    queryKey: ["provider-models"],
    queryFn: fetchers.providerModels,
    refetchInterval: 60000,
  });

  if (provLoading) return <div className="text-[var(--text-secondary)]">Loading providers…</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Server} label="Total Providers" value={provData?.count ?? 0} />
        <StatCard icon={CheckCircle2} label="Active" value={active} sub={errored > 0 ? `${errored} erroring` : "all healthy"} />
        <StatCard icon={AlertTriangle} label="In Backoff" value={inBackoff} sub={errored > 0 ? `${errored} errored` : "none"} />
        <StatCard icon={Boxes} label="Models Available" value={modelData?.count ?? provData?.modelsAvailable ?? "—"} />
      </div>

      {/* Provider table */}
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)] overflow-hidden">
        <div className="px-5 py-3 border-b border-[var(--border)]">
          <h2 className="text-sm font-semibold text-[var(--text-secondary)]">Provider Connections</h2>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {providers.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-tertiary)]/40">
                    <td className="px-5 py-3">
                      <div className="font-mono text-xs">{shortName(p.id, p.provider)}</div>
                      <div className="text-[10px] text-[var(--text-secondary)] truncate max-w-[220px]">{p.baseUrl || "—"}</div>
                    </td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3 text-[var(--text-secondary)]">{p.modelCount}</td>
                    <td className="px-5 py-3">
                      {p.backoffLevel > 0 ? (
                        <span className="text-yellow-400 font-mono">{p.backoffLevel}</span>
                      ) : (
                        <span className="text-[var(--text-secondary)]">0</span>
                      )}
                    </td>
                    <td className="px-5 py-3 text-xs text-[var(--text-secondary)] max-w-[260px] truncate" title={p.lastError ?? ""}>
                      {p.lastError ?? "—"}
                    </td>
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)] p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-[var(--text-secondary)]">Available Models</h2>
          <span className="text-xs text-[var(--text-secondary)]">{models.length} models</span>
        </div>
        {modelsLoading ? (
          <div className="text-[var(--text-secondary)] text-sm">Loading models…</div>
        ) : models.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {models.map((m) => (
              ...
              >
                <div className="font-mono text-xs break-all leading-relaxed">{m.id}</div>
                {m.owned_by && (
