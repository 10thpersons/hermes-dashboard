function renderMarkdown(md: string): string {
  ...
  html = html.replace(
    /`([^`]+)`/g,
    '<code class="bg-[var(--bg-tertiary)] px-1.5 py-0.5 rounded text-sm font-mono text-[var(--accent)]">$1</code>',
  );
  ...
  html = html.replace(
    /^> (.+)$/gm,
    '<blockquote class="border-l-2 border-[var(--accent)] pl-3 my-2 text-[var(--text-secondary)]">$1</blockquote>',
  );
  ...
  const result: string[] = [];
  const blockTags =
    /^<(h[1-3]|pre|li|blockquote|hr|table|thead|tbody|tr|th|td|ul|ol)/;
